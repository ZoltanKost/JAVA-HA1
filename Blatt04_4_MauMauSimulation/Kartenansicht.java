...
class Kartenansicht
{
    ...
    /**
     * Initialisiert eine neue leere Kartenansicht mit dem Titel "Testansicht".
     */
    public Kartenansicht()
    {
        new JFXPanel();
        Platform.setImplicitExit(false);
        Platform.runLater( () -> init("Testansicht") );
    }
    
    /**
     * Initialisiert eine neue leere Kartenansicht mit einem Titel.
    ...
    /**
     * Zeigt drei Karten an.
     * @param dreier ein Objekt vom Typ DreiKarten, das drei Karten liefern kann.
     */
    private void zeige(DreiKarten dreier)
    {
        Platform.runLater( () -> {
                _root.getChildren().clear();
                _root.getChildren().add(macheTextAusKarte(dreier.karte1().bild()));
                _root.getChildren().add(macheTextAusKarte(dreier.karte2().bild()));
                _root.getChildren().add(macheTextAusKarte(dreier.karte3().bild()));
            } );
    }

    private Text macheTextAusKarte(Kartenbild karte)
    {
        Text t = new Text(gibStringVonCodepoint(karte.codepoint()));
        t.setFont(new Font(96));
        Color farbe = Color.BLACK;
        if (karte.farbe() == Kartenfarbe.HERZ || karte.farbe() == Kartenfarbe.KARO)
        {
            farbe = Color.RED;
        }
        t.setFill(farbe);
