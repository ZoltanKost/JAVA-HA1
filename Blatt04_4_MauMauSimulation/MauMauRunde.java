...
class MauMauRunde
{
    ...
    public KartenTripel zieheDreiBilder(Kartenansicht ansicht,
                                        Kartenstapel stapel)
    {
        stapel = new Kartenstapel(_kartensatz);
        
        KartenTripel result = new KartenTripel();
        
        while(true)
        {
            if(stapel.anzahlKarten() < 3) stapel = new Kartenstapel(_kartensatz);
            Spielkarte karte = stapel.obersteKarteZiehen();
            _spieler1.nimmKarte(karte);
            System.out.println(result.tryAdd(karte));
            if(result.istVollstaendig()) break; 
            
            karte = stapel.obersteKarteZiehen();
            _spieler2.nimmKarte(karte);
            System.out.println(result.tryAdd(karte));
            if(result.istVollstaendig()) break; 

            karte = stapel.obersteKarteZiehen();
            _spieler3.nimmKarte(karte);
            System.out.println(result.tryAdd(karte));
            if(result.istVollstaendig()) break;
        }
        return result;
